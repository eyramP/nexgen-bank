...
async def startup_health_check(timeout: float = 90.0) -> float:
    try:
        ...
        logger.error(f"Error during health startup check: {e}")
        return False
...
async def lifespan(app: FastAPI):
    try:
        ...
        logger.info("All services intialized and healthy")
        yield
    except Exception as e:
